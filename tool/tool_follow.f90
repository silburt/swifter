!A.S. modified code so that the arguments must be passed as command line. But then can mass produce as a result.
...
! Internals
     LOGICAL(LGT)                                    :: lxdr
     INTEGER(I4B)                                    :: npl,ntp,ntp0,nsp,iout,idump,iloop,i,iu,ifol,nskp,ierr,iout_form,ic,istep,id,fileno
     REAL(DP)                                        :: t,tfrac,tbase,tmax
     CHARACTER(STRMAX)                               :: inparfile, tempifol, tempnskp, filename, tempfileno
...
! Executable code
     ...
     READ (tempnskp, *) nskp        !A.S.
     READ (tempfileno, *) fileno    !A.S.
    !READ (*,100)inparfile
 100 FORMAT(A)
     ...
     SELECT CASE (out_form)
          ...
          CASE ("FILT")
               iout_form=FILT
     END SELECT
     WRITE(filename,'(a,i4.4,a)') "follow",fileno,".txt"        !A.S.
     OPEN(UNIT=7,FILE=filename)                                 !A.S.
